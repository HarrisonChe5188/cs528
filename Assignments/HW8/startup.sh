...
if [ "$ROLE" = "webserver" ]; then
    ...
    if ! download_from_gcs "${SCRIPTS_BUCKET}" "client.py" "./client.py"; then
        echo "Failed to fetch client.py"
        touch "$LOCKFILE"
        exit 1
    fi
    chown "$USERNAME:$USERNAME" ./client.py || true
    chmod +x ./client.py || true

    # Also grab the old binary client if present (best-effort)
    if download_from_gcs "${SCRIPTS_BUCKET}" "http-client" "./http-client" 2>/dev/null; then
        chown "$USERNAME:$USERNAME" ./http-client || true
        chmod +x ./http-client || true
        echo "http-client binary also downloaded"
    fi

    # client.py needs google-cloud-storage (already in venv from earlier install)
    # Write a convenience wrapper so you can just run 'runclient' from SSH
    cat > "/usr/local/bin/runclient" <<'WRAPPER'
#!/bin/bash
source /home/0979h/venv/bin/activate
python /home/0979h/service/client.py "$@"
WRAPPER
    chmod +x /usr/local/bin/runclient

    echo "client.py downloaded and ready. Run with: runclient -d <LB_IP> -b <bucket> -w <folder> -n <num> -p 80 -v"
    touch "$LOCKFILE"
    exit 0
